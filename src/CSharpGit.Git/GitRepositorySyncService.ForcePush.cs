...
internal sealed partial class GitRepositorySyncService
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        if (GitOperationDetector.Detect(repository) != RepositoryOperation.None)
            throw new InvalidOperationException(
                "Complete or abort the current Git operation before force pushing.");

        if ((remote is null) != (remoteBranch is null))
            throw new ArgumentException(
                "Explicit force-push target requires both remote and remote branch.");

        var localBranch = await CurrentBranchAsync(
            repository,
            cancellationToken);
        GitRefValidator.Validate(localBranch, nameof(localBranch));

        var localRef = $"refs/heads/{localBranch}";
        var localCommit = await _runner.RunAsync(
        ...
        if (remote is null)
        {
            remote = await _runner.RunOptionalAsync(
                repository.WorkingDirectory,
                cancellationToken,
                "config",
                "--get",
                $"branch.{localBranch}.remote");
            var mergeRef = await _runner.RunOptionalAsync(
                repository.WorkingDirectory,
                cancellationToken,
                "config",
                "--get",
        ...
        await EnsureRemoteExistsAsync(
            repository,
            remote,
            cancellationToken);
        var pushDestination = await ReadSinglePushDestinationAsync(
            repository,
            remote,
            cancellationToken);

        var remoteRef = $"refs/heads/{remoteBranch}";
        var remoteQuery = await RunGitCapturedAsync(
            repository,
            cancellationToken,
            "ls-remote",
            "--exit-code",
        ...
        if (remoteQuery.ExitCode != 0)
            throw CreateGitFailure(
                "Could not query the force-push destination",
                remoteQuery);
        ...
        if (matchingLines.Count != 1)
            throw new RepositoryOpenException(
                "Git ls-remote did not return exactly one matching remote branch ref.");

        var expectedRemoteCommit = matchingLines[0][0];
        GitRefValidator.ValidateObjectId(
            expectedRemoteCommit,
            nameof(expectedRemoteCommit));

        var localSubject = await _runner.RunAsync(
            repository.WorkingDirectory,
            cancellationToken,
            false,
            "show",
        ...
        var objectCheck = await RunGitCapturedAsync(
            repository,
            cancellationToken,
            "cat-file",
            "-e",
        ...
        {
            var subject = await _runner.RunOptionalAsync(
                repository.WorkingDirectory,
                cancellationToken,
                "show",
                "-s",
                "--format=%s",
                expectedRemoteCommit);
            remoteSubject = string.IsNullOrWhiteSpace(subject)
                ? null
                : subject;
        }
        ...
        {
            throw CreateGitFailure(
                "Could not inspect the remote commit locally",
                objectCheck);
        }
        ...
        return new ForcePushWithLeaseSnapshot(
            ...
            remoteBranch,
            expectedRemoteCommit,
            string.IsNullOrWhiteSpace(localSubject)
                ? null
                : localSubject,
            remoteSubject);
    }
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        if (GitOperationDetector.Detect(repository) != RepositoryOperation.None)
            throw new ForcePushWithLeaseCancelledException(
                "A Git operation started after confirmation was prepared. Start the force-push workflow again.");

        var currentBranch = await CurrentBranchAsync(
            repository,
            cancellationToken);

        if (!string.Equals(
                currentBranch,
                snapshot.LocalBranch,
                StringComparison.Ordinal))
            throw new ForcePushWithLeaseCancelledException(
                "The current branch changed after confirmation was prepared. Start the operation again.");

        var currentLocalCommit = await _runner.RunAsync(
            repository.WorkingDirectory,
            cancellationToken,
            true,
            "rev-parse",
            "--verify",
            $"refs/heads/{snapshot.LocalBranch}");
        ...
        if (!string.Equals(
                ...
                StringComparison.Ordinal))
            throw new ForcePushWithLeaseCancelledException(
                $"{snapshot.LocalBranch} changed after confirmation was prepared. Review the new state and start the operation again.");

        string currentPushDestination;
        try
        {
            currentPushDestination = await ReadSinglePushDestinationAsync(
                repository,
                snapshot.Remote,
                cancellationToken);
        }
        ...
        {
            throw new ForcePushWithLeaseCancelledException(
                $"The push destination changed after confirmation was prepared: {exception.Message}");
        }

        if (!string.Equals(
                currentPushDestination,
                snapshot.RemotePushDestination,
                StringComparison.Ordinal))
            throw new ForcePushWithLeaseCancelledException(
                "The remote push destination changed after confirmation was prepared. Start the operation again.");

        await _pushExecutor.RunAsync(
            repository,
            cancellationToken,
            BuildForcePushArguments(snapshot));
    }

    internal static string[] BuildForcePushArguments(
        ForcePushWithLeaseSnapshot snapshot)
    {
        ArgumentNullException.ThrowIfNull(snapshot);
        ValidateSnapshot(snapshot);
    ...
    internal static PushResultKind ClassifyPushFailure(string output) =>
        GitPushExecutor.ClassifyFailure(output);
        ...
        ForcePushWithLeaseSnapshot snapshot)
    {
        GitRefValidator.Validate(
            snapshot.LocalBranch,
            nameof(snapshot.LocalBranch));
        GitRefValidator.Validate(
            snapshot.Remote,
            nameof(snapshot.Remote));
        GitRefValidator.Validate(
            snapshot.RemoteBranch,
            nameof(snapshot.RemoteBranch));
        GitRefValidator.ValidateObjectId(
            snapshot.LocalCommit,
            nameof(snapshot.LocalCommit));
        GitRefValidator.ValidateObjectId(
            snapshot.ExpectedRemoteCommit,
            nameof(snapshot.ExpectedRemoteCommit));

        if (string.IsNullOrWhiteSpace(snapshot.RemotePushDestination))
            throw new ArgumentException(
                "The force-push snapshot has no push destination.",
                nameof(snapshot));
    }

    private async Task EnsureRemoteExistsAsync(
        Repository repository,
        string remote,
    ...
    private async Task<string> ReadSinglePushDestinationAsync(
        Repository repository,
        string remote,
        CancellationToken cancellationToken)
    {
        ...
        var result = await RunGitCapturedAsync(
            repository,
            cancellationToken,
            "remote",
            "get-url",
        ...
        if (result.ExitCode != 0)
            throw CreateGitFailure(
                $"Could not resolve push URL for remote '{remote}'",
                result);

        var destinations = result.StandardOutput.Split(
            ['\r', '\n'],
            StringSplitOptions.RemoveEmptyEntries
            | StringSplitOptions.TrimEntries);
    ...
    private static RepositoryOpenException CreateGitFailure(
        string prefix,
        GitCommandResult result)
    {
        var detail = GitPushExecutor.JoinGitOutput(result);
