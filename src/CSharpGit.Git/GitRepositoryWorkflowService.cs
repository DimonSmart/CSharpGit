...
internal sealed class GitRepositoryWorkflowService : IRepositoryWorkflowService
{
    ...
    public Task ChooseConflictSideAsync(Repository repository, ConflictFile conflict, ConflictResolutionSide side, CancellationToken cancellationToken = default)
    {
        ...
        var operation = _operationDetector.Detect(repository);
        ...
        return _commands.RunMutationAsync(repository, cancellationToken, "checkout", gitSide, "--", conflict.Path);
    }
    ...
    public Task KeepConflictDeletionAsync(Repository repository, ConflictFile conflict, CancellationToken cancellationToken = default)
    {
        ...
        return _commands.RunMutationAsync(repository, cancellationToken, "rm", "--", conflict.Path);
    }
    ...
    public Task StageResolvedConflictAsync(Repository repository, ConflictFile conflict, CancellationToken cancellationToken = default)
    {
        ...
        return _commands.RunMutationAsync(repository, cancellationToken, "add", "--", conflict.Path);
    }
    ...
    public Task CreateStashAsync(Repository repository, string? message = null, CancellationToken cancellationToken = default)
    {
        ...
        return _commands.RunMutationAsync(repository, cancellationToken, arguments.ToArray());
    }
    ...
    public Task ApplyStashAsync(Repository repository, string stashName, CancellationToken cancellationToken = default)
    {
        ...
        return _commands.RunMutationAsync(repository, cancellationToken, "stash", "apply", stashName);
    }

    public Task PopStashAsync(Repository repository, string stashName, CancellationToken cancellationToken = default)
    {
        ValidateStashName(stashName);
        return _commands.RunMutationAsync(repository, cancellationToken, "stash", "pop", stashName);
    }
    ...
    public async Task<MergeResult> MergeAsync(Repository repository, string branch, CancellationToken cancellationToken = default)
    {
        ...
        var before = await _commands.RunOptionalAsync(repository.WorkingDirectory, cancellationToken, "rev-parse", "--verify", "HEAD");
        try
        {
            await _commands.RunMutationAsync(repository, cancellationToken, "merge", "--no-edit", branch);
        }
        ...
        var after = await _commands.RunOptionalAsync(repository.WorkingDirectory, cancellationToken, "rev-parse", "--verify", "HEAD");
        ...
        var parents = await _commands.RunAsync(repository.WorkingDirectory, cancellationToken, true, "show", "-s", "--format=%P", "HEAD");
    ...
    {
        GitReferenceValidator.ValidateRefName(onto, nameof(onto));
        var resolvedOnto = await _commands.RunAsync(repository.WorkingDirectory, cancellationToken, true, "rev-parse", "--verify", $"{onto}^{{commit}}");
        var output = await _commands.RunAsync(repository.WorkingDirectory, cancellationToken, false, "log", "--reverse", "--format=%H%x00%s%x1e", $"{resolvedOnto}..HEAD");
    ...
    public async Task<RebaseResult> StartInteractiveRebaseAsync(Repository repository, InteractiveRebasePlan plan, CancellationToken cancellationToken = default)
    {
        ...
        if (_operationDetector.Detect(repository) != RepositoryOperation.None) throw new InvalidOperationException("Complete or abort the current Git operation first.");
        ...
        var supportDirectory = GitRebaseSupport.DirectoryPath(repository);
        try
        {
            ...
            if (_operationDetector.Detect(repository) != RepositoryOperation.Rebase)
                GitRebaseSupport.Cleanup(repository);
            return result;
        }
        catch
        {
    ...
    {
        if (_operationDetector.Detect(repository) != RepositoryOperation.Rebase) throw new InvalidOperationException("No rebase is in progress.");
    ...
    {
        await _commands.RunMutationAsync(repository, cancellationToken, "rebase", "--abort");
        GitRebaseSupport.Cleanup(repository);
    }
    ...
    private async Task<RebaseResult> RunRebaseCommandAsync(Repository repository, IReadOnlyDictionary<string, string?> environment, CancellationToken cancellationToken, params string[] arguments)
    {
        try
        {
            await _commands.RunAsync(repository.WorkingDirectory, cancellationToken, false, environment, GitCommandKind.User, arguments);
    ...
    private static void ValidateRebasePlan(InteractiveRebasePlan plan)
    {
        ...
        {
            GitReferenceValidator.ValidateObjectName(item.Commit);
    ...
    private async Task RunOperationCommandAsync(Repository repository, string action, CancellationToken cancellationToken)
    {
        ...
        if (action == "continue" && state.Operation is RepositoryOperation.Merge or RepositoryOperation.CherryPick or RepositoryOperation.Revert)
        {
            ...
            var messageEditor = Path.Combine(supportDirectory, OperatingSystem.IsWindows() ? "message-editor.cmd" : "message-editor.sh");
            await WriteNoOpEditorAsync(messageEditor, cancellationToken);
            await _commands.RunAsync(repository.WorkingDirectory, cancellationToken, false,
        ...
        await _commands.RunMutationAsync(repository, cancellationToken, command, $"--{action}");
    }

    private static void ValidateConflictAction(ConflictFile conflict, bool allowed)
    {
        ArgumentNullException.ThrowIfNull(conflict);
        GitPathValidator.ValidatePath(conflict.Path);
