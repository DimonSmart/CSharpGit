...
public sealed class BatchStagingTests : IDisposable
{
    ...
    public async Task StageFilesStagesMixedSelectedPathsIncludingRenameSpacesAndUnicode()
    {
        ...
        var service = services.WorkingTree;
        var repository = await services.Repositories.OpenAsync(_temporaryDirectory);
        var changes = (await services.State.ReadAsync(repository)).Changes;
        ...
        var refreshed = (await services.State.ReadAsync(repository)).Changes;
    ...
    public async Task UnstageFilesOnlyResetsSelectedIndexEntriesAndPreservesWorkingTreeBytes()
    {
        ...
        var service = services.WorkingTree;
        var repository = await services.Repositories.OpenAsync(_temporaryDirectory);
        var before = await services.State.ReadAsync(repository);
        ...
        var after = (await services.State.ReadAsync(repository)).Changes;
    ...
    public async Task SameFileCanMoveBetweenStagedAndUnstagedWithoutLosingWorkingTreeContent()
    {
        ...
        var service = services.WorkingTree;
        var repository = await services.Repositories.OpenAsync(_temporaryDirectory);
        var both = Assert.Single((await services.State.ReadAsync(repository)).Changes);
        Assert.True(both.IsStaged && both.IsUnstaged);

        await service.UnstageFilesAsync(repository, [both]);

        var unstaged = Assert.Single((await services.State.ReadAsync(repository)).Changes);
        ...
        var staged = Assert.Single((await services.State.ReadAsync(repository)).Changes);
    ...
    public async Task UnstageFilesInUnbornRepositoryKeepsModifiedWorkingTreeVersion()
    {
        ...
        var service = services.WorkingTree;
        var repository = await services.Repositories.OpenAsync(_temporaryDirectory);
        var change = Assert.Single((await services.State.ReadAsync(repository)).Changes);
    ...
    public async Task UnstageAllResetsIndexToHeadWithoutChangingWorkingTree()
    {
        ...
        Assert.Equal(oneBytes, File.ReadAllBytes(Path.Combine(_temporaryDirectory, "one.cs")));
        Assert.Equal(twoBytes, File.ReadAllBytes(Path.Combine(_temporaryDirectory, "two.cs")));
        Assert.DoesNotContain((await services.State.ReadAsync(repository)).Changes, change => change.IsStaged);
    }
