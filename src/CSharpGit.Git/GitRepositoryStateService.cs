...
internal sealed class GitRepositoryStateService : IRepositoryStateService
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var headReference = await _commands.RunOptionalAsync(
            ...
            "--short",
            "HEAD");
        var headCommit = await _commands.RunOptionalAsync(
            repository.WorkingDirectory,
            cancellationToken,
            "rev-parse",
            "--verify",
            "HEAD");
        var status = await _commands.RunAsync(
            repository.WorkingDirectory,
            cancellationToken,
            false,
            "status",
            ...
            "-z",
            "--untracked-files=all");
        var globalConfig = await _commands.RunAsync(
            repository.WorkingDirectory,
            cancellationToken,
            false,
            "config",
            ...
            "--null",
            "--list");
        var localConfig = await _commands.RunAsync(
            repository.WorkingDirectory,
            cancellationToken,
            false,
            "config",
        ...
        CancellationToken cancellationToken)
    {
        ...
        var output = await _commands.RunAsync(
            repository.WorkingDirectory,
            cancellationToken,
            false,
            "for-each-ref",
        ...
        var remoteNames = await _commands.RunAsync(
            repository.WorkingDirectory,
            cancellationToken,
            false,
            "remote");
        ...
        {
            var fetchUrl = await _commands.RunAsync(
                repository.WorkingDirectory,
                cancellationToken,
                false,
                "remote",
                "get-url",
                name);
            var pushUrl = await _commands.RunAsync(
                repository.WorkingDirectory,
                cancellationToken,
                false,
                "remote",
        ...
        return new GitReferences(local, remote, remotes, []);
    }
        ...
        CancellationToken cancellationToken)
    {
        ...
        var unmerged = await _commands.RunAsync(
            repository.WorkingDirectory,
            cancellationToken,
            false,
            "ls-files",
        ...
        AddConflictPathsFromMessage(
            ...
            knownPaths);
        AddConflictPathsFromMessage(
            Path.Combine(repository.GitDirectory, "rebase-merge", "message"),
            knownPaths);
        AddConflictPathsFromMessage(
            Path.Combine(repository.GitDirectory, "rebase-apply", "final-commit"),
            knownPaths);
        ...
        foreach (var path in knownPaths.Order(StringComparer.Ordinal))
        {
            ...
            conflicts.Add(
                new ConflictFile(
                    ...
                    && !isBinary
                    && File.Exists(
                        Path.Combine(repository.WorkingDirectory, path)),
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            var blob = await _commands.RunOptionalAsync(
                repository.WorkingDirectory,
                cancellationToken,
                "show",
                $":{stage}:{path}");
