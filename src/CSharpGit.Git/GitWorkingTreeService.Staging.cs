...
internal sealed partial class GitWorkingTreeService
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        return _commands.RunMutationAsync(repository, cancellationToken, arguments);
    }
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        {
            await _commands.RunMutationAsync(repository, cancellationToken, ["restore", "--staged", "--", .. paths]);
            return;
        }

        await _commands.RunMutationAsync(repository, cancellationToken, ["rm", "--cached", "--force", "--ignore-unmatch", "--", .. paths]);
    }
    ...
    public async Task UnstageAllAsync(Repository repository, CancellationToken cancellationToken = default)
    {
        ...
        if (await HasHeadAsync(repository, cancellationToken))
        {
            await _commands.RunMutationAsync(repository, cancellationToken, "reset", "--mixed");
            return;
        }
        ...
        await _commands.RunMutationAsync(repository, cancellationToken, "rm", "--cached", "-r", "--force", "--ignore-unmatch", "--", ".");
    }

    private async Task<bool> HasHeadAsync(Repository repository, CancellationToken cancellationToken) =>
        await _commands.RunOptionalAsync(repository.WorkingDirectory, cancellationToken, "rev-parse", "--verify", "HEAD") is { Length: > 0 };
