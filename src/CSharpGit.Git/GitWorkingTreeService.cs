...
internal sealed partial class GitWorkingTreeService : IWorkingTreeService
{
    ...
    internal GitWorkingTreeService(GitRepositoryCommandRunner runner)
    {
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        if (await _runner.RunOptionalAsync(
                ...
                "HEAD") is { Length: > 0 })
        {
            await _runner.RunMutationAsync(
                repository,
                cancellationToken,
                PathArguments("restore", change, "--staged"));
        }
        else
        {
            await _runner.RunMutationAsync(
