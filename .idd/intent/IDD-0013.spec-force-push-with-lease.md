...
- Missing remote branch не создаётся force workflow: для создания используется
  ordinary `Publish branch`. Первая публикация никогда автоматически не включает
  force; non-fast-forward publish может перейти только в существующий explicit
  force-with-lease workflow. Несколько push destinations не поддерживаются force
  workflow.
